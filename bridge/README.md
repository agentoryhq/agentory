...
The bridge is a desktop application that connects the **Agentory** server (remote) to the **local MCP programs** installed on your PC, such as FreeCad, filesystem tools, local databases and any other MCP server that runs as a `stdio` process.
...
```
[Local program]  ←stdio→  [Bridge]  ←WebSocket→  [Agentory server]  ←→  [AI agent]
...
|---------|-------------------|
| Windows | `Agentory-Bridge-Setup-x.x.x.exe` |
| macOS (Apple Silicon) | `Agentory-Bridge-x.x.x-arm64.dmg` |
| macOS (Intel) | `Agentory-Bridge-x.x.x.dmg` |
| Linux | `Agentory-Bridge-x.x.x.AppImage` |
...
**macOS** — double-clicking shows *"Agentory Bridge cannot be opened because the
...
- from a terminal: `xattr -dr com.apple.quarantine "/Applications/Agentory Bridge.app"`.
...
```bash
chmod +x Agentory-Bridge-x.x.x.AppImage
./Agentory-Bridge-x.x.x.AppImage
```
...
Open the Agentory web interface in your browser and go to **Settings → Account** (or **Profile**).  
Copy the JWT token of your session.

> **Quick alternative:** you can copy it from the browser's localStorage.  
> Open DevTools → Application → Local Storage → `agentory-store` → look for the `token` field.
...
|-------|---------------|---------|
| **Server URL** | WebSocket address of the Agentory server | `wss://yourserver.com` or `ws://localhost:3000` locally |
...
- Verify that the Agentory server is running and reachable
