#!/usr/bin/env bash
# backup.sh — snapshot of Agentory's persistent data.
#
# Produces  backups/agentory-backup-<timestamp>/  containing:
...
DB_NAME="$(get_env DB_NAME)"; DB_NAME="${DB_NAME:-agentory}"
...
TS="$(date +%Y%m%d-%H%M%S)"
OUT="$ROOT/backups/agentory-backup-$TS"
mkdir -p "$OUT"
