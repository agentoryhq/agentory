#!/usr/bin/env bash
# install.sh — guided installer for the containerized startup of Agentory.
#
...
echo "${B}╭───────────────────────────────────────────────╮${N}"
echo "${B}│   Agentory · containerized installer   │${N}"
echo "${B}╰───────────────────────────────────────────────╯${N}"
...
if (( IS_PROD )); then
  ...
  # comma-separated (e.g. https://agentory.example.com).
...
cat <<EOF
  Skills and the sandbox run code. Choose how much to isolate them:
    ...
    ${B}2) Isolated${N}      ${DIM}— + broker: every execution is an ephemeral,
                     ...
                     the backend on agentory-internal, no internet). Recommended.${N}
...
if [[ "$LEVEL" == "1" ]]; then
  ...
  if [[ "$LEVEL" == "3" ]]; then
    ...
    ok "Level 2 · Isolated (broker; jobs reach the backend on agentory-internal, no internet)"
  fi
  ...
  # Baseline 'agentory-internal' (backend, no-WAN) is ALWAYS on. Here we optionally
  ...
  echo
  echo "  ${DIM}Baseline: every skill job reaches the backend on 'agentory-internal' (no internet).${N}"
