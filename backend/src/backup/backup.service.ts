...
export class BackupService {
  ...
  constructor(cfg: ConfigService) {
    this.db = {
      ...
      name: cfg.get<string>('DB_NAME', 'agentory'),
    };
  ...
  async createBackup(): Promise<BackupInfo> {
    ...
    const work = await mkdtemp(join(tmpdir(), 'agentory-backup-'));
    try {
      ...
      const filename = `agentory-backup-${stamp}.tar`;
  ...
  private isBackupName(f: string): boolean {
    return /^agentory-backup-[\w.-]+\.tar$/.test(f);
  }
