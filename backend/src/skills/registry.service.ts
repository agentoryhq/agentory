...
const OFFICIAL_REGISTRY_URL =
  'https://raw.githubusercontent.com/agentoryhq/agentory-skills/main/registry.json';
  ...
  async fetchIndex(): Promise<RegistryIndex> {
    ...
    try {
      const res = await fetch(this.registryUrl, {
        headers: { 'Accept': 'application/json', 'User-Agent': 'Agentory/1.0' },
  ...
  async downloadZip(downloadUrl: string): Promise<Buffer> {
    ...
    try {
      const res = await fetch(downloadUrl, {
        headers: { 'User-Agent': 'Agentory/1.0' },
